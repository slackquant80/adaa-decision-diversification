# ADAA v0.15 — parent / counterfactual decision-panel export
# PURPOSE:
...
cat("ADAA v0.15 parent / counterfactual decision export\n")
...
monthly_wide <- data.frame(signal_month=month_key, coredata(prices_m), check.names=FALSE)
...
write_weights <- function(w, tag, trace=NULL) {
  ...
  x <- data.frame(signal_month=month_key[keep], coredata(w[keep,]), check.names=FALSE)
