...
def laa_dashboard_row_alignment(observation_month: date, trading_dates: Sequence[date]) -> LaaTiming:
    """Current dashboard row-pairs UNRATE observation t with market month-end t.

    PerformanceAnalytics applies an xts weight dated at month-end immediately after that close,
    so the first holding date is the first return date after the weight date.
    """
    obs = month_start(observation_month)
    release = month_start(add_months(obs, 1))
    weight_date = month_end(obs)
