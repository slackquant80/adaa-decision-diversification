...
build_haa <- function() {
  ...
  for (i in 13:nrow(prices_m)) {
    ...
    tp <- sum(as.numeric(prices_m[i,"TIP"] / prices_m[i-c(1,3,6,12),"TIP"] - 1))
...
write_w <- function(w, tag) {
  ...
  write.csv(x, file.path(out_dir, paste0("G2_", tag, "_TARGET_WEIGHTS_R_v0_11.csv")), row.names=FALSE)
}

write_w(build_haa(), "HAA")
write_w(build_baa(), "BAA_AGGRESSIVE")
write_w(build_adm(), "ADM")
write_w(build_faa(), "FAA_LEGACY")
write_w(build_laa(), "LAA")

cat("PASS: performance-blind R target-weight exports written. No portfolio performance was calculated.\n")
