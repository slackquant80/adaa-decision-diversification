...
matplotlib.rcParams.update({
    ...
    "svg.hashsalt": "adaa-exact-exhibit-v1.1.4",
})

FIGURES = {
    "Figure 1": ("FIGURE_1_SOURCE_DATA_v0.22.csv", "Figure_1_Return_Correlation_vs_Decision_Timing_PAPER_v1.24_EXACT"),
    "Figure 2": ("FIGURE_2_SOURCE_DATA_v0.22.csv", "Figure_2_What_When_HowMuch_Decision_Fingerprints_PAPER_v1.24_EXACT"),
    "Figure 3": ("FIGURE_3_SOURCE_DATA_v0.22.csv", "Figure_3_Different_Decision_Clocks_PAPER_v1.24_EXACT"),
    "Figure 4": ("FIGURE_4_SOURCE_DATA_v0.23.csv", "Figure_4_Rolling_60M_Hindsight_Winner_PAPER_v1.24_EXACT"),
    "Figure 5": ("FIGURE_5_SOURCE_DATA_v1.0.1.csv", "Figure_5_Broad_Plateau_Unstable_Optimum_PAPER_v1.24_EXACT"),
    "Figure 6": ("FIGURE_6_SOURCE_DATA_v1.1.csv", "Figure_6_Cumulative_Wealth_PAPER_v1.24_EXACT"),
    "Figure 7": ("FIGURE_7_SOURCE_DATA_v1.1.csv", "Figure_7_Drawdown_Depth_and_Duration_PAPER_v1.24_EXACT"),
    "Figure 8": ("FIGURE_8_SOURCE_DATA_v1.0.1.csv", "Figure_8_Stress_Protection_and_Rapid_Reversal_PAPER_v1.24_EXACT"),
    "Appendix Figure Z1": ("FIGURE_Z1_SOURCE_DATA_FULL_2023_PAIRWISE_v1.0.csv", "Figure_Z1_Full_2023_Strategy_Zoo_Decision_Distance_PAPER_v1.24_EXACT"),
    "Appendix Figure Z2": ("FIGURE_Z2_SOURCE_DATA_FULL_2023_FIVE_RULE_SELECTOR_v1.0.csv", "Figure_Z2_Full_2023_Five_Rule_Score_Distribution_PAPER_v1.24_EXACT"),
    "Appendix FX Figure 1": ("FIGURE_FX1_SOURCE_DATA_MONTHLY_PATHS_v1.0.csv", "Figure_FX1_Currency_Exposure_Growth_PAPER_v1.24_EXACT"),
    "Appendix FX Figure 2": ("FIGURE_FX2_SOURCE_DATA_v1.1.csv", "Figure_FX2_Threshold_Sensitivity_PAPER_v1.24_EXACT"),
}
...
def figure_2(df: pd.DataFrame, out: Path) -> None:
    ...
    ax.set_xlabel("What: average number of active holdings")
    ax.set_ylabel("When: months with a target-weight change (%)")
    ax.set_xlim(1.55, 6.25)
    ax.set_ylim(-2, 90)
    ax.text(0.02, 0.02, "How much: marker area represents average rule-defined defensive allocation.",
...
def figure_5(df: pd.DataFrame, out: Path) -> None:
    ...
    ax1.scatter(x, df.later_practitioner_weight, marker="D", s=40, label="Later practitioner weights", zorder=4)
    ...
    ax2.plot([], [], color=bootstrap_color, linewidth=2.1, label="Bootstrap optimum: 2.5-97.5%")
...
def figure_6(df: pd.DataFrame, out: Path) -> None:
    ...
    for label in order:
        ...
        ax.plot(g.holding_month, g.growth_of_1, linewidth=1.6, label=label)
...
def figure_7(df: pd.DataFrame, out: Path) -> None:
    label_map = {
        ...
        "HAA hindsight strongest sleeve": "HAA hindsight strongest sleeve",
        "60/40 SPY/IEF": "60/40 SPY/IEF",
        "SPY": "SPY",
    }
...
def figure_z2(df: pd.DataFrame, out: Path) -> None:
    ...
    ax.axvline(hist.mean_decision_distance, linestyle="--", linewidth=1.25,
               label=f"Historical 2023 set (rank {int(hist['rank']):,}/{len(df):,})")
    ax.axvline(best.mean_decision_distance, linestyle=":", linewidth=1.25,
               label="Maximum-diversity set")
...
def figure_fx2(df: pd.DataFrame, out: Path) -> None:
    ...
    ax.text(hj, li - 0.36, "Legacy", ha="center", va="top", fontsize=7.2, fontweight="bold")
...
def main() -> None:
    ...
    manifest = args.manifest or (output_dir / "EXACT_FIGURE_RENDER_MANIFEST_v1.1.4.csv")
