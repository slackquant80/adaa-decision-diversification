#!/usr/bin/env python3
"""ADAA v0.26: corrected drawdown definition for inference/stress/benchmark outputs.\n\nThis is a narrow re-run of the frozen v0.19 engine. It changes only drawdown initialization: initial portfolio wealth W0=1 is included as a valid running peak. All return paths, rules, weights, samples, costs, bootstrap seeds and non-drawdown statistics are unchanged.\n\nOriginal v0.19 description: dependence-aware inference, stress/failure decomposition, and benchmark expansion.
...
def max_dd(x):
    w=np.cumprod(1+np.asarray(x,float)); peak=np.maximum.accumulate(np.r_[1.0,w])[1:]; dd=w/peak-1
...
def bootstrap_pair(a,b,rf,block,B,seed):
    ...
    peakA=np.maximum.accumulate(np.concatenate([np.ones((B,1)),wealthA],axis=1),axis=1)[:,1:]
    peakB=np.maximum.accumulate(np.concatenate([np.ones((B,1)),wealthB],axis=1),axis=1)[:,1:]
    ddA=wealthA/peakA-1; ddB=wealthB/peakB-1
...
def main():
    ...
    pd.concat(allpaths,ignore_index=True).to_csv(out/'G6_ALL_MONTHLY_PATHS_FOR_INFERENCE_v0_26_RECHECK.csv',index=False)
    ...
    for k in ['ADAA_equal20','ADAA_historical','Benchmark_SPY','Benchmark_60_40_SPY_IEF','Benchmark_60_40_VTI_BND','Benchmark_60_40_SPY_AGG','Benchmark_BIL','Sleeve_HAA']:
        ...
        for bps in [0,25]:
            ...
            bench.append(m)
    pd.DataFrame(bench).to_csv(out/'G6_EXTERNAL_BENCHMARK_FAMILY_DD_CORRECTED_v0_26.csv',index=False)
    ...
    pd.DataFrame(inf).to_csv(out/'G6_DEPENDENCE_AWARE_INFERENCE_DD_CORRECTED_v0_26.csv',index=False)
    pd.DataFrame(sens).to_csv(out/'G6_BLOCK_LENGTH_SENSITIVITY_DD_RECHECK_v0_26.csv',index=False)
    ...
    pd.DataFrame(erows).to_csv(out/'G6_STRESS_EPISODE_PERFORMANCE_DD_CORRECTED_v0_26.csv',index=False)
    ...
    pd.DataFrame(sync).to_csv(out/'G6_STRESS_DECISION_SYNCHRONIZATION_v0_26_RECHECK.csv',index=False)
    ...
    pd.DataFrame(contrib).to_csv(out/'G6_STRESS_SLEEVE_ATTRIBUTION_v0_26_RECHECK.csv',index=False)
    ...
    pd.DataFrame(rows).to_csv(out/'G6_DATA_DEFINED_FAILURE_WINDOWS_v0_26_RECHECK.csv',index=False)

    # Evidence record.
    record={
        'version':'v0.26-drawdown-correction','primary_holding_window':[holding[0],holding[-1]],'rules_reselected':False,'weights_reselected':False,'benchmark_shopping':False,
    ...
    (out/'G6_DRAWDOWN_CORRECTED_INFERENCE_RECORD_v0_26.json').write_text(json.dumps(record,indent=2),encoding='utf-8')
    print('PASS: v0.26 corrected-drawdown inference/stress/benchmark re-run complete.')
