# ADAA v0.17 independent local-R validation of the opened performance/accounting engine.
...
metrics <- function(x, rf) {
  ...
  n <- length(x); wealth <- cumprod(1+x); cagr <- wealth[n]^(12/n)-1
  annmean <- mean(x)*12; vol <- sd(x)*sqrt(12); ex <- x-rf; sh <- mean(ex)/sd(ex)*sqrt(12)
  dd <- wealth/cummax(wealth)-1; j <- which.min(dd); mdd <- dd[j]; cal <- cagr/abs(mdd)
  ...
  c(months=n,CAGR=cagr,annualized_arithmetic_mean=annmean,annualized_volatility=vol,BIL_excess_Sharpe=sh,
    ...
    worst_1m_return=as.numeric(w1[1]),worst_3m_return=as.numeric(w3[1]),worst_12m_return=as.numeric(w12[1]),worst_36m_return=as.numeric(w36[1]),ending_growth_of_1=wealth[n])
}
...
for (nm in names(P)) {
  ...
  for (bps in cost_grid) {
    ...
    mm <- metrics(x,rf)
    rows[[z]] <- data.frame(portfolio=nm,cost_bps=bps,t(mm),total_gross_L1_turnover=sum(p$gross_L1_turnover),annualized_mean_gross_L1_turnover=mean(p$gross_L1_turnover)*12,check.names=FALSE)
    z<-z+1
  }
}
out <- do.call(rbind,rows)
write.csv(out,file.path(out_dir,'G6_PRIMARY_PERFORMANCE_R_VALIDATION_v0_17.csv'),row.names=FALSE)
...
write.csv(monthly,file.path(out_dir,'G6_PRIMARY_MONTHLY_RETURN_TURNOVER_R_VALIDATION_v0_17.csv'),row.names=FALSE)
cat('PASS: v0.17 independent base-R performance/accounting validation exports written.\n')
