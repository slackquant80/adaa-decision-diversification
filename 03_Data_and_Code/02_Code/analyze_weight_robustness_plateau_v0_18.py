#!/usr/bin/env python3
"""ADAA v0.18 — moderate-optimization / broad-plateau robustness diagnostics.
...
AR=asset_ret.loc[holding].reindex(columns=assets).fillna(0.0).to_numpy()
...
refdf.to_csv(OUT/'G6_WEIGHT_PLATEAU_REFERENCE_POINTS_v0_18.csv',index=False)
...
pd.DataFrame(summary).to_csv(OUT/'G6_WEIGHT_SIMPLEX_100K_DISTRIBUTION_v0_18.csv',index=False)
...
for gap in [.01,.025,.05,.10]:
    ...
    for i,k in enumerate(SLEEVES):
        ...
        rec[f'{k}_max']=float(simplex[mask,i].max()) if mask.any() else np.nan
    bands.append(rec)
pd.DataFrame(bands).to_csv(OUT/'G6_WEIGHT_NEAR_OPTIMAL_BANDS_v0_18.csv',index=False)
...
pd.DataFrame(local_rows).to_csv(OUT/'G6_HISTORICAL_WEIGHT_LOCAL_PERTURBATION_v0_18.csv',index=False)
...
rolldf.to_csv(OUT/'G6_ROLLING_60M_OPTIMAL_WEIGHT_PATH_v0_18.csv',index=False)
...
pd.DataFrame(rsummary).to_csv(OUT/'G6_ROLLING_60M_OPTIMAL_WEIGHT_SUMMARY_v0_18.csv',index=False)
...
for b in range(N_BOOT):
    ...
    boot.append({'bootstrap_id':b+1,**{f'w_{k}':float(w[i]) for i,k in enumerate(SLEEVES)},
                 ...
                 'L1_distance_to_equal':float(np.abs(w-EQUAL).sum())})
    x0=w
bootdf=pd.DataFrame(boot); bootdf.to_csv(OUT/'G6_BLOCK_BOOTSTRAP_OPTIMAL_WEIGHTS_v0_18.csv',index=False)
bsummary=[]
for k in SLEEVES:
    x=bootdf[f'w_{k}']
...
pd.DataFrame(bsummary).to_csv(OUT/'G6_BLOCK_BOOTSTRAP_OPTIMAL_WEIGHT_SUMMARY_v0_18.csv',index=False)
...
pd.DataFrame(control_rows).to_csv(OUT/'G6_ROLLING_OPTIMIZER_CHASING_CONTROL_v0_18.csv',index=False)
...
ctrl=pd.read_csv(OUT/'G6_ROLLING_OPTIMIZER_CHASING_CONTROL_v0_18.csv')
record={
    'version':'v0.18','status':'SUPPORTIVE_ROBUSTNESS_EVIDENCE',
...
(OUT/'G6_ROBUST_ARCHITECTURE_WEIGHT_PLATEAU_RECORD_v0_18.json').write_text(json.dumps(record,indent=2),encoding='utf-8')
