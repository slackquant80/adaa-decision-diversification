# ADAA Public Replication Run Order v1.0.1

**Release status: CLEAN-RUN VALIDATED. Follow this order without reselection or optimization.**


## Environment and expected runtime
Run the frozen workflow from a package/project root named exactly `05_ADAA`, because the frozen scripts fail closed on another root name. If you clone the GitHub repository for execution, specify the destination explicitly, for example `git clone <repository-url> 05_ADAA`, or clone normally and rename the local clone to `05_ADAA` before running the workflow. The repository name itself does not need to be `05_ADAA`.
...
`python 00_VALIDATE_PUBLIC_PACKAGE_v1_0_1.py`
...
Reconcile regenerated headline metrics, tables and 12 active exhibits against the frozen public paper. Then rerun the package validator, scan generated content for raw/private files, and freeze the final redistribution manifest/checksum.