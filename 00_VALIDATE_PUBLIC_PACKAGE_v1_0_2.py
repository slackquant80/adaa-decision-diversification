...
for p in files:
    ...
    if rel not in {'00_VALIDATE_PUBLIC_PACKAGE_v1_0.py','00_VALIDATE_PUBLIC_PACKAGE_v1_0_1.py','00_VALIDATE_PUBLIC_PACKAGE_v1_0_2.py'} and p.suffix.lower() in {'.py','.r','.md','.txt','.csv','.json','.yml','.yaml'} and p.stat().st_size < 5_000_000:
...
manifest=root/'MANIFEST_SHA256_v1.0.2.csv'
if manifest.exists():
...
print('PUBLIC_PACKAGE_V1_0_2_VALIDATION:', 'PASS' if not fail else 'FAIL')
